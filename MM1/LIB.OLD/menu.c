 ...
 * Copyright (C) 1993,94,95 by Allen C. Huffman of Sub-Etha Software
 *
 * Note: expects users' program to have About(wpath); and Help(wpath);
 *
 * 0.00 09/18/93 - Library version.
 * 0.01 09/19/93 - addition to passing pointer to "ch" variable...
 * 0.02 09/20/93 - addition to passing "click" status variable...
 * 0.03 06/02/94 - change pull-down menu scaling (be nice to mice) and
 *                 correctly support 8 menu options
 * 1.00 03/10/95 - MM/1 updates, MBPrint routines
 *      03/28/95 - Add "maclike" menu operation for MM/1 version, modify
 *                 MouseClick() to return # of clicks waiting, if any,
 *                 moved MouseClick to mouse.c file.
 * 1.01 04/05/95 - made return ch=NULL if no keypress detected
...
brute_force:
   ...
   if (_gs_rdy(wpath)>0) { /* character waiting... */
...
#endif
#ifdef TCWIN /* TCWin looks for ESC first */
      if (*ch==27) { /* if ESC, read next char for menu */
         read(0,ch,1);
         *ch = *ch | 128; /* set high bit, faking an Alt-Key */
      }
      if (*ch == 12) { /* ^L = refresh window */
         ShowAll();
      }
#endif
      ...
      *ch='\0'; /* nothing waiting, return NULL */
   } /* end of "if (_gs_rdy>0)" */
   return(FALSE);
}
...
{
#ifdef TCWIN
   MBPrint(wpath,2,TRUE,"About");
#else
   MBPrint(wpath,2,TRUE,"\x1f\x22A\x1f\x23bout");
#endif
   About(wpath);
#ifdef TCWIN
   MBPrint(wpath,2,FALSE,"About");
#else
...
{
#ifdef TCWIN
   MBPrint(wpath,74,TRUE,"Help");
#else
   MBPrint(wpath,74,TRUE,"\x1f\x22H\x1f\x23elp");
#endif
   Help(wpath);
#ifdef TCWIN
   MBPrint(wpath,74,FALSE,"Help");
#else
   ...
   do {
      *option=42; /* temp value... */
      ...
      do {
         ...
         Snooze(wpath); 
#ifdef TCWIN
         if (_gs_rdy(0)>0) { /* character waiting */
            read(0,&ch,1);
#else
         if (_gs_rdy(wpath)>0) { /* character waiting */
            read(wpath,&ch,1);
