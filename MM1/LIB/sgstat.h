...
struct _sgs {
	...
	char
		...
		_sgs_baud;		/* baud rate */
	short _sgs_d2p;		/* offset to second device name string */
	char _sgs_xon,		/* x-on char */
		 _sgs_xoff,		/* x-off char */
		 _sgs_tabcr,	/* tab character */
		 _sgs_tabsiz;	/* tab size */
	long _sgs_tbl;		/* Device table address (copy) */
	short _sgs_col;		/* Current column number */
...
struct _sgr {
	char
		...
		_sgr_fill1;		/* not used */
	short _sgr_cyls;	/* number of cylinders (tracks) */
	char _sgr_sides,	/* number of sides */
		_sgr_verify;	/* 0 = verify on writes */
	short _sgr_spt,		/* default sectors per track */
	...
	short _sgr_ssize,	/* sector size (bytes) */
		_sgr_cntl;		/* device control word */
	char _sgr_trys,		/* number of tries counter (1=no-retry/correction) */
		_sgr_lun;		/* SCSI LUN of device */
	short _sgr_wpc,		/* start write-precompensation cylinder */
		_sgr_rwr,		/* start reduced write current cylinder */
		_sgr_park;		/* parking cylinder */
	long _sgr_lsnoffs;	/* LSN offset for partition */
	short _sgr_totcyls;	/* total cylinders on device */
	char _sgr_ctrlrid;	/* SCSI controller ID */
	char _sgr_res2;		/* reserved */
	unsigned _sgr_scsiopt;	/* SCSI driver options flags */
	unsigned _sgr_maxcnt;	/* maximum byte-count passable to driver */
	char _sgr_rsvd[5],	/* reserved for future use */
