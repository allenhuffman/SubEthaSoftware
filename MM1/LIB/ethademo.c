/*
 * EthaDemo.c V1.02 by Allen C. Huffman
 * Copyright (C) 1993,94,95 by Sub-Etha Software
 *
 ...
 * 1.02 09/26/95 - Updates for revised TCWin compatible library
 */
...
#endif
#ifdef TCWIN
   PrintAt(wpath,4,15,"KEYBOARD - Alt+LETTER of menu to select, or <ESC>,LETTER.  Once the");
#else
...
#else
   PrintAt(wpath,4,19,"menu, or press ESC to abort a menu. (up=^P dn=^N lf=^B rt=^f)");
#endif
...
Usage()
{
   fputs("\nEthaDemo V1.02 by Allen Huffman (coco-sysop@genie.geis.com)\n",stderr);
   fputs("Copyright (C) 1994,95 by Sub-Etha Software\n\n",stderr);
...
int wpath;
{
   ...
   PrintAt(wpath,2,1,"EthaDemo V1.02 by Allen C. Huffman");
