 ...
 *      03/01/96 - Fixed annoying "always re-reads directory whether you want
 *                 it to or not" bug.  :)  Thanks James!
 */
...
/*                  if ((fileflag[i+start])==TRUE) PrintAt(wpath,PLACE-2,i+START,"*");
         ...
         if (CheckMenu(wpath,&menu,&option,&ch,&click)==TRUE) {
            ...
            if (Process(wpath,menu,option)==TRUE) { /* process options */
               loop=FALSE; /* must re-read directory if here */
            }
            continue; /* go to loop check */
         }
#ifndef NOMOUSE
