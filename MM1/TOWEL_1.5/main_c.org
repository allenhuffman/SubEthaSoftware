 ...
 * 1.04 6/07/95 - malloc space for directory names and stuff (FAST dir!)
 */
...
char *dirbuf; /* pointer to where we store the directory */
char **filename; /* pointer to pointers to structure for file data */
long *filestart; /* pointer to longs or something */
...
char *argv[];      /* argv[1-?] = argurments */
{
   ...
   do { /* open and read directory here */
      ...
      dirbuf=(char *)malloc(dirsize); /* malloc space for directory */
      if (dirbuf==NULL) MallocError(wpath);
      ...
      filename=(char **)malloc(sizeof(char *) * dirsize); /* space for dirname */
      if (filename==NULL) {
         free(dirbuf);
         MallocError(wpath);
      }
      ...
      for (files=0,j=0; j<dirsize; j++) { /* loop through entries */
         if (dirbuf[files*32]!=0) { /* if not deleted files, */
            filename[files]=dirbuf+files*32; /* point filename there */
            filestart[files]=0; /* not implemented yet */
            fileflag[files]=FALSE; /* not flagged */
            for (j=0; j<28; j++) {
      ...
      } /* here, filename[] points to the file names and stuff */
      if (sortdir) {
