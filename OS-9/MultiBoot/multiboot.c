/*
 * MultiBoot.c V1.05 by Allen C. Huffman
 ...
 * 1.05 9/09/94 - Fix "already allocated" message
 */
...
/* rest of code goes here... */
   ...
   fputs("\nMultiBoot V1.05 by Allen Huffman (OS-9) and Terry Todd (RS-DOS).\n",stderr);
...
int fpath,sides;
{
   ...
   for (i=0;i<5;i++) {
      ...
      if ( (buffer[byte] & bit) >0 ) { /* if any bits are set, */
         fputs("\n\nBootInstall: Required sectors already allocated.  Re-run on a freshly formatted\n",stderr);
